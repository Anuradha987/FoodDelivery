...
const Restaurant=({route, navigation})=>{
    ...
    const [orderItems, setOrderItems] = React.useState([]);  //for the + and - functionality
    ...
    //for the + and - functionality
    function editOrder(action, menuId, price){

        let orderList = orderItems.slice()  //duplicate items by using slice method
        let item = orderList.filter(a => a.menuId == menuId)  //filter based on menuId

        if(action =="+"){
          
           if(item.length>0){    //if already added item to the cart (>0) then increase the qty
               let newQty = item[0].qty+1 
               item[0].qty = newQty
               item[0].total = item[0].qty * price
           }
           else{              //if this is not added to the cart yet, create a new object
            const newItem = {
                menuId : menuId, 
                qty : 1, 
                price: price, 
        ...
        else{     //filter the order list based on the menuId.  For the - sign
                if(item.length>0){
                    if(item[0]?.qty > 0){
                        let newQty = item[0].qty - 1
                        item[0].qty = newQty               //assign the newqty value
                        item[0].total = newQty * price     //recalculate total
                    }
                }
                setOrderItems(orderList)
            }
    ...
    function getOrderQty(menuId) {    //set the initial value to 0
    ...
    function sumOrder(){   //calculate total price of the cart
        let total = orderItems.reduce((a,b)=> a+ (b.total || 0), 0)
        return total.toFixed(2)
    }

    function getBasketItemCount(){          //sum up all quantity
        let itemCount = orderItems.reduce((a,b) => a + (b.qty || 0),0)  //items in cart's initial value set to 0
        return itemCount
    }
        ...
        return (
            <View>
                ...
                <View style={{ backgroundColor: COLORS.white, borderTopLeftRadius: 40, borderTopRightRadius: 40 }}>
                    ...
                    {/* Order Button */}
                    <View style={{padding: SIZES.padding, alignItems: 'center',justifyContent: 'center' }}>
